...
export class MultiplayerComponent implements OnDestroy {
  ...
  setupLobby(): void {
    ...
    this.racers = [
      {
        id: 'user', name: user?.username || 'You', avatar: user?.avatar || '🎯',
        progress: 0, wpm: 0, isUser: true, finished: false, targetWpm: 0
      },
      ...this.BOT_NAMES.map((b, i) => ({
        id: `bot_${i}`, name: b.name, avatar: b.avatar,
        progress: 0, wpm: 0, isUser: false, finished: false,
        targetWpm: 45 + Math.floor(Math.random() * 55)
      }))
    ];
    this.typedValue = ''; this.userWpm = 0; this.userProgress = 0;
    this.userFinished = false; this.totalKeystrokes = 0; this.errorCount = 0;
    this.accuracy = 100; this.userRank = 0; this.elapsedSec = 0;
    this.saveError = ''; this.status = 'lobby';
  }

  startRace(): void {
    this.status = 'countdown'; this.countdown = 3;
  ...
  beginRace(): void {
    this.status = 'racing'; this.startTime = Date.now();
    ...
    this.racers.filter(r => !r.isUser).forEach(bot => {
      ...
      const interval = setInterval(() => {
        if (bot.finished || this.status === 'finished') { clearInterval(interval); return; }
        ...
        if (charsTyped >= this.currentText.length && !bot.finished) {
          bot.finished = true; bot.progress = 100; bot.finishTime = elapsed;
          clearInterval(interval); this.checkAllFinished();
        }
      }, 100);
  ...
  onInput(event: Event): void {
    ...
    if (typed.length >= this.currentText.length && !this.userFinished) {
      ...
      userRacer.finished = true; userRacer.progress = 100;
  ...
  updateUserStats(): void {
    ...
    const userRacer = this.racers.find(r => r.isUser)!;
    userRacer.wpm = this.userWpm; userRacer.progress = this.userProgress;
  }

  checkAllFinished(): void {
    const finishedCount = this.racers.filter(r => r.finished).length;
    if (finishedCount >= this.racers.length - 1) setTimeout(() => this.finishRace(), 1500);
  }
  ...
  finishRace(): void {
    ...
    // Save to backend via HttpClient
    this.api.saveRace(this.userWpm, this.accuracy, this.errorCount, Math.round(this.elapsedSec))
      .subscribe({
        next: () => {},
  ...
  playAgain(): void {
    clearInterval(this.timerInterval); this.botIntervals.forEach(clearInterval);
    this.botIntervals = []; this.setupLobby();
  }
